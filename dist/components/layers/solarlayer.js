import React, { useRef, useEffect } from '../../pkg/react.js';
import { useSelector } from '../../pkg/react-redux.js';
import { CSSTransition } from '../../pkg/react-transition-group.js';
import { range, last } from '../../pkg/lodash.js';
import styled from '../../pkg/styled-components.js';
import parse from '../../pkg/color-parse.js';
import { useWidthObserver, useHeightObserver } from '../../hooks/viewport.js';
import { useSolarEnabled } from '../../hooks/router.js';
import { stackBlurImageOpacity } from '../../helpers/image.js';
import { solarColor } from '../../helpers/scales.js';
import { useInterpolatedSolarData } from '../../hooks/layers.js';
const maxSolar = last(solarColor.domain());

const solarIntensityToOpacity = intensity => Math.floor(intensity / maxSolar * 255);

const opacityToSolarIntensity = opacity => Math.floor(opacity * maxSolar / 255); // Pre-process solar color components across all integer values
...
export default (({
  ...
  const isVisible = enabled && isMapLoaded && !isMoving; // Render the processed solar forecast image into the canvas.
  ...
  useEffect(() => {
    if (ref.current && isVisible && solar && width && height) {
      ...
      const {
        lo1,
        la1
      } = solar.header; // Project solar data onto the image opacity channel
      ...
      stackBlurImageOpacity(image, 0, 0, width, height, 10 * mapZoom); // Map image opacity channel onto solarColor scale to get the real solar colors.
