import React, { useRef, useState, useEffect, useMemo } from '../../pkg/react.js';
...
import { useWindEnabled } from '../../hooks/router.js';
import Windy from '../../helpers/windy.js';
import { useInterpolatedWindData } from '../../hooks/layers.js';
...
export default (({
  ...
  const viewport = useMemo(() => {
    ...
    return [[[0, 0], [width, height]], width, height, [sw, ne]];
  }, [unproject, width, height]); // Kill and reinitialize Windy every time the layer visibility changes, which
  ...
  useEffect(() => {
    if (!windy && isVisible && ref.current && interpolatedData) {
      ...
      w.start(...viewport); // Set in the next render cycle.
