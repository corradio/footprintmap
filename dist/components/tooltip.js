...
const Tooltip = ({
  ...
  const style = {};
  let x = 0;
  let y = 0; // Check that tooltip is not larger than screen
  ...
  if (2 * MARGIN + width >= screenWidth || position.x + width + MARGIN >= screenWidth && position.x - width - MARGIN <= 0) {
    ...
    x = position.x + MARGIN; // Check that tooltip does not go over the right bound
  ...
  style.transform = `translate(${x}px,${y}px)`; // Don't show the tooltip until its dimensions have
