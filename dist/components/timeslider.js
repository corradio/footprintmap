...
const createChangeAndInputHandler = (datetimes, onChange, setAnchoredTimeIndex) => ev => {
  ...
  let index = sortedIndex(datetimes.map(t => t.valueOf()), value); // If the slider is past the last datetime, we set index to null in order to use the scale end time.
...
export default connect(state => ({
  datetime: state.data.grid.datetime
}))(TimeSlider);