...
const AreaGraphLayers = React.memo(({layers, datetimes, timeScale, valueScale, mouseMoveHandler, mouseOutHandler, isMobile, svgRef}) => {
  const [x1, x2] = timeScale.range();
  const [y2, y1] = valueScale.range();
  if (x1 >= x2 || y1 >= y2) {
    return null;
  }
  const layerArea = area().x((d) => timeScale(d.data.datetime)).y0((d) => valueScale(d[0])).y1((d) => valueScale(d[1])).defined((d) => Number.isFinite(d[1]));
  let mouseOutTimeout;
  const handleLayerMouseMove = (ev, layerIndex) => {
    if (mouseOutTimeout) {
      clearTimeout(mouseOutTimeout);
      mouseOutTimeout = void 0;
    }
