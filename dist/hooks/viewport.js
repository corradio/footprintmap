...
export function useWidthObserver(ref, offset = 0) {
  const [width, setWidth] = useState(0); // Resize hook
  ...
  useEffect(() => {
    ...
    updateWidth(); // Update container width on every resize
...
export function useHeightObserver(ref, offset = 0) {
  const [height, setHeight] = useState(0); // Resize hook
  ...
  useEffect(() => {
    ...
    updateHeight(); // Update height on every resize
...
export function useWindowSize() {
  const [windowSize, setWindowSize] = useState({}); // Resize hook
  ...
  useEffect(() => {
    ...
    updateSize(); // Add window listeners
