...
import { interpolate } from '../pkg/d3-interpolate.js';
import { values } from '../pkg/lodash.js';
import moment from '../pkg/moment.js';
import { getRefTime, getTargetTime } from '../helpers/grib.js';
import { useCustomDatetime } from './router.js';
export function useExchangeArrowsData() {
  ...
  return useMemo(() => isConsumption ? values(exchanges).filter(d => d.lonlat && d.sortedCountryCodes) : [], [isConsumption, exchanges]);
}
export function useInterpolatedWindData() {
  const windData = useSelector(state => state.data.wind);
  const customDatetime = useCustomDatetime(); // TODO: Recalculate every 5 minutes if custom datetime is not set.
  ...
  return useMemo(() => {
    ...
    return [{ ...gribs1[0],
      data: gribs1[0].data.map((d, i) => interpolate(d, gribs2[0].data[i])(k))
    }, { ...gribs1[1],
      data: gribs1[1].data.map((d, i) => interpolate(d, gribs2[1].data[i])(k))
    }];
...
export function useInterpolatedSolarData() {
  ...
  return useMemo(() => {
    ...
    return { ...grib1,
      data: grib1.data.map((d, i) => interpolate(d, grib2.data[i])(k))
    };
