...
*/
import { windColor } from './scales.js';
...
var Windy = function (params) {
  ...
  var TRANSPARENT_BLACK = [255, 0, 0, 0]; // interpolation for vectors like wind (u,v,m)
  ...
  var distort = function (λ, φ, x, y, scale, wind, windy) {
    var u = wind[0] * scale;
    var v = wind[1] * scale;
    var d = distortion(λ, φ, x, y, windy); // Scale distortion vectors by u and v, then add.
  ...
  var animate = function (bounds, field) {
    ...
    function draw() {
      var deltaMs = Date.now() - lastFrameTime; // 16 ms ~ 60 fps
      // if we take any longer than that, then scale the opacity
      // inversely with the time

      var b = deltaMs < 16 ? 1 : 16 / deltaMs; // Fade existing particle trails.

      g.globalCompositeOperation = "destination-in"; // This is the parameter concerning the fade property/bug

      g.globalAlpha = Math.pow(0.9, b);
      g.fillRect(bounds.x, bounds.y, bounds.width, bounds.height); // Prepare for drawing a new particle

      g.globalCompositeOperation = "source-over";
      g.globalAlpha = 1; // Draw new particle trails.
