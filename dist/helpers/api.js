...
import { sha256 } from '../pkg/js-sha256.js';
...
export function protectedJsonRequest(path) {
  ...
  return new Promise((resolve, reject) => {
    request.json(url).header('electricitymap-token', Cookies.get('electricitymap-token')).header('x-request-timestamp', timestamp).header('x-signature', sha256(token + path + timestamp)).get(null, (err, res) => {
      if (err) {
        reject(err);
  ...
  if (err) {
    if (err.target) {
      const {
        ...
        status,
        statusText
      } = err.target; // Avoid catching HTTPError 0
      ...
      if (!status) return; // Also ignore 5xx errors as they are usually caused by server downtime and are not useful to track.
