...
import { CARBON_INTENSITY_DOMAIN, FOSSIL_FUEL_KEYS } from './constants.js';
export function getElectricityProductionValue({
  ...
  const value = isStorage ? -storage : production; // If the value is not defined but the capacity
...
export function getExchangeCo2Intensity(mode, zoneData, electricityMixMode) {
  ...
  return exchange > 0 ? exchangeCo2Intensity : electricityMixMode === 'consumption' ? zoneData.co2intensity : zoneData.co2intensityProduction;
}
...
function getEnergyRatio(electricityMixMode, data, filter) {
  ...
  const keyTotal = electricityMixMode === 'consumption' ? 'totalPrimaryEnergyConsumptionTWh' : 'totalPrimaryEnergyProductionTWh';
  ...
  return Object.keys(data[key]).filter(filter).map(k => data[key][k]).reduce((a, b) => a + b, 0) / data[keyTotal];
}
